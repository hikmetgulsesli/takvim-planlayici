...
export function EventDetailModal({ event, isOpen, onClose, onEdit, onDelete }: EventDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[var(--color-surface)]/80 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-lg glass-morphism rounded-lg shadow-[0_32px_64px_rgba(128,131,255,0.12)] border border-[var(--color-outline-variant)]/20 p-8 relative">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        {event.description && (
          <div className="mb-6">
            ...
            <p className="text-[var(--color-on-surface)] leading-relaxed bg-[var(--color-surface-container)]/50 rounded-lg p-4">
              {event.description}
            </p>
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2 text-[var(--color-on-surface)]">
            <span className="material-symbols-outlined text-[var(--color-primary)]">
              {event.reminder === 'none' ? 'notifications_off' : 'notifications'}
            </span>
        ...
        <div className="flex gap-3 pt-4 border-t border-[var(--color-outline-variant)]/20">
          <button
            ...
            className="flex-1 px-6 py-4 rounded-lg font-bold text-[var(--color-error)] hover:bg-[var(--color-error)]/10 transition-all duration-200 text-sm flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="flex-[2] bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-container)] text-[var(--color-on-primary)] px-6 py-4 rounded-lg font-bold shadow-xl shadow-[var(--color-primary)]/20 hover:scale-[1.02] active:scale-95 transition-all duration-200 text-sm flex items-center justify-center gap-2"
          >
